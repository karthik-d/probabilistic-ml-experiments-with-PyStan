functions{
    ...
    matrix getAttemptProbabilties(int nq, int ns, int[,] skillset_cases, matrix needed, real p_knows, vector p_guesses){
        ...
        for (i in 1:cases){
            for (j in 1:nq){
                attempt_prob[i,j] = (req_neg_sum[i,j]==0 ? p_knows : p_guesses[j]);
            }
        }
    ...
    vector evaluateLikelihoods(int nq, int ns, int[,] skillset_cases, matrix needed, real p_knows, vector p_guesses, int[] isCorrect){
        int cases = power(2, ns);
        matrix[cases, nq] attempt_prob;   // Either knows or guesses
...
data{
    ...
    vector<lower=0,upper=1>[num_questions] p_guesses;
}
